...
(function($) {
    function rgb2hex(rgb) {
        ...
        return rgb && 4 === rgb.length ? "#" + ("0" + parseInt(rgb[1], 10).toString(16)).slice(-2) + ("0" + parseInt(rgb[2], 10).toString(16)).slice(-2) + ("0" + parseInt(rgb[3], 10).toString(16)).slice(-2) : ""
    }
    ...
    mcw.numberFormat = function(num, iso, shorten = !1, decimals = "auto") {
        ...
        decimals = shorten ? format.decimals : "auto" == decimals ? num >= 1 ? format.decimals : num < 1e-6 ? 14 : 6 : parseInt(decimals);
    ...
    $.fn.drawChart = function() {
        ...
        depp.require("mcw-chartjs", function() {
            ...
            if (0 === gradient) var background = "rgba(" + color + "," + opacity + ")";
            else {
                background = self[0].getContext("2d").createLinearGradient(0, 0, 0, gradient);
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(tooltipItem, data) {
                                    return mcw.priceFormat(tooltipItem.parsed.y, currency).replace(/(<([^>]+)>)/gi, "")
                                },
            ...
            depp.require("mcw-echarts", function() {
                ...
                function getData(callback) {
                    if ("chart" === opts.type) {
                        ...
                        url += "day" === opts.period || "week" === opts.period ? "histohour" : "histoday";
                        url += "?fsym=" + opts.symbol + "&tsym=USDT";
                        url += "all" === opts.period ? "&allData=true" : "&limit=" + periods.candlestick[opts.period];
                        url += "&aggregate=1&extraParams=massivecrypto";
                        stname = "mcw-" + opts.symbol.toLowerCase() + "-usd-candlestick-" + opts.period
                    }
                    ...
                    getData(function(data) {
                        ...
                        if ("chart" === opts.type)
                            for (var i = 0; i < data.prices.length; i++) {
                                ...
                                date = "day" == opts.period ? formatAMPM(date) : months[date.getMonth()] + " " + date.getDate() + ", " + date.getFullYear();
                                var value = data.prices[i][1] * opts.rate;
                                labels.push(date);
                                values.push(value)
                            } else
                                for (i = 0; i < data.Data.length; i++) {
                                    date = new Date(1e3 * data.Data[i].time);
                                    date = "day" == opts.period ? formatAMPM(date) : months[date.getMonth()] + " " + date.getDate() + ", " + date.getFullYear();
                                    value = "chart" == opts.type ? data.Data[i].close * opts.rate : [data.Data[i].close * opts.rate, data.Data[i].open * opts.rate, data.Data[i].low * opts.rate, data.Data[i].high * opts.rate];
                                    labels.push(date);
                                    values.push(value);
                ...
                var options = {
                        ...
                        smooth: "true" == self.attr("data-smooth"),
                ...
                options = {
                    ...
                    grid: {
                        left: 15,
                        right: 15,
                        top: self.width() > breakpoint ? 80 : 110,
                        bottom: 10,
                        containLabel: !0
                    },
                    tooltip: {
                        ...
                        formatter: function(params) {
                            ...
                            if ("chart" == opts.type) {
                                ...
                                tooltip += "<br/>";
                                tooltip += params[1].marker + ' V: <b style="color: #fff;">' + mcw.numberFormat(params[1].value, opts.currency, !1, 0) + "</b> " + opts.coin
                            }
                            return tooltip
                        }
                    },
                    ...
                    options.yAxis.push({
                        ...
                        axisLabel: {
                            ...
                            formatter: function(value) {
                                return 0 == value ? 0 : mcw.numberFormat(value, opts.currency)
                            }
                        },
                        splitLine: {
                            show: !0,
    ...
    $.fn.mcwConverter = function() {
        ...
        if (0 !== this.length) {
            ...
            "" != fromdefault && optionone[0].selectize.setValue(fromdefault);
            "" != todefault && optiontwo[0].selectize.setValue(todefault);
            fieldone.val(1);
            calcdown();
            if (auto) {
                ...
                button.click(function(e) {
                    e.preventDefault();
                    "down" == direction ? calcdown() : calcup()
                })
            }
        }
    };
    ...
    $.fn.mcwTabs = function() {
        var self = this,
            tabs = self.find(".mcw-tab"),
            items = self.find(".mcw-tab-content");
        tabs.click(function() {
    ...
    $(".mcw-box").each(function() {
        ...
        if ($(this).hasClass("mcw-box-2")) {
            ...
            self.find("select").change(function() {
                ...
                percent >= 0 ? self.find(".mcw-list-change").toggleClass("down up") : self.find(".mcw-list-change").toggleClass("up down")
            })
        }
    });
    ...
    $(window).on("load", function() {
        $(".cc-stats").each(function() {
            ...
            var itemcount = $(this).find(".cc-coin").length,
                ...
                speed = $(this).closest(".mcw-ticker").data("speed"),
                duration = 10 * itemsize;
            if (200 === speed) duration = 10;
            else if (0 == speed) duration = 0;
            else if (speed > 100) {
                speed -= 100;
                ...
                speed = 100 - speed;
                speed = speed / 10 * (8 * itemsize);
                duration += speed
            }
            speed = itemcount * duration / 1e3;
    ...
    $.fn.coinmcResize = function() {
        ...
        width >= 992 ? breakpoint = "lg" : width >= 768 ? breakpoint = "md" : width >= 576 && (breakpoint = "sm");
